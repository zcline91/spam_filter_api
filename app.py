...
def text_request():
    ...
    resp.set_cookie('spam_prob', 
                    f"{100 * response_df.loc[0,'spam_probability']:.3f}")
    resp.set_cookie('ham_prob', 
                    f"{100 * (1 - response_df.loc[0,'spam_probability']):.3f}")
    return resp
...
def results():
    ...
    return render_template("results.html", spam_prob=spam_prob, 
                           ham_prob=ham_prob)
    ...
    if archive and allowed_archive(archive.filename):
        ...
        for filename in os.scandir(td.name):
            with open(filename.path, 'rb') as eml:
                ...
                frames.append(
                    handle_request(email_obj, "file", return_prob=True)
                    .to_dict(orient='records')
                ) # in future, pass list to handlerequests and receive 1 df
    return(frames)


def handle_request(email_input, model_type, return_prob=True, 
                   return_inputs=False):
    if model_type == "text":
        model = text_model
        response_df = pd.DataFrame(
            {"pred": model.predict(email_input).astype(bool)})
        if return_prob:
            response_df['spam_probability'] = \
                model.predict_proba(email_input)[:,1]
    ...
    if return_inputs and model_type == "text":
        response_df['subject_head'] = (email_input['subject']
                                       .str.slice(stop=100))
...
def retrieve_probas(e, input_type):
    if input_type == "text":
        ...
        output_str = (f"Probabilities: {probs[0]*100:.2f}% likely to be ham "
            f"and {probs[1]*100:.2f}% likely to be spam.")
